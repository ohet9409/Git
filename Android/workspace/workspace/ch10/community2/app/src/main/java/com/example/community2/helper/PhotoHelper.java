package com.example.community2.helper;
...
public class PhotoHelper {
    ...
    public Bitmap getThumb(Activity activity, String path){
        ...
        if (deviceWidth < deviceHeigh) maxScale = deviceHeigh;
        ...
        if (fscale > maxScale){
            ...
            //  이미지 읽어오기 - sampleSize만큼 크기를 줄어서 읽어온다.
    ...
    public Bitmap rotate(Bitmap bitmap, int degrees){
        if (degrees != 0 && bitmap != null){
            // The Matrix class hids a 3x3 matrix for transforming coordinates.
            Matrix m = new Matrix();
            //  public void setRotate(float degrees, float px, float py) : bitmap 기준, 위치와 각도 설정
