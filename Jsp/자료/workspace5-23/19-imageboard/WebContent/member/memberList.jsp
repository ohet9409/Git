...
<% } %>	
	...
	<tr>
		...
		<% if(startPage > 3) { %>
			[<a id="paging" href="../main/index.jsp?req=memberList&pg=<%=startPage-1%>">이전</a>]
		<% } %>
			
		<% for(int i=startPage; i<=endPage ; i++) { %>
			<% if(pg == i) {	// 현재페이지 %>
			[<a id="currentPaging" href="../main/index.jsp?req=memberList&pg=<%=i%>"><%=i%></a>]
			<% } else { %>
			[<a id="paging" href="../main/index.jsp?req=memberList&pg=<%=i%>"><%=i%></a>]
			<% } %>
		<% } %>
	
		<% if(endPage < totalP) { %>			
			[<a id="paging" href="../main/index.jsp?req=memberList&pg=<%=endPage+1%>">다음</a>]
		<% } %>
		</td>
	</tr>
</table>
